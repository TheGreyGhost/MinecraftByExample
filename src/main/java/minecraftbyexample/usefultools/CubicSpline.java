 ...
 *
 * Typical usage:
 * 1) Create an array of points that you want to interpolate [x,y]
 * 2) call createMonotoneCubicSpline to create the spline for those points [x,y]
 * 3) call interpolate to calculate the interpolated y for any given x
 *
 */
public class CubicSpline {
   ...
   */
  public static CubicSpline createMonotoneCubicSpline(List<Float> x, List<Float> y) {
  ...
  /**
   * Interpolates the value of Y = f'(X) for given X, i.e. the first derivative. Clamps X to the domain of the spline.
   *
   * @param x The X value.
   * @return The interpolated Y = f'(X) value.
   */
  public float interpolateFirstDerivative(float x) {
    // Handle the boundary cases.
    final int n = mX.size();
    if (Float.isNaN(x)) {
      return x;
  ...
  (3*d-6*c+3*b+6*a)*x^2+(-2*d+6*c-4*b-6*a)*x+b
  where

          (mYgeti * (1 + 2 * t) + h * mMi * t) * (1 - t) * (1 - t)
          + (mYgetip1 * (3 - 2 * t) + h * mMip1 * (t - 1)) * t * t

  a = mYgeti, b = mMi, c = mYgeip1, d = mMip1
  ...
  private CubicSpline(List<Float> x, List<Float> y, float[] m) {
    mX = x;
    mY = y;
    mM = m;
  }
