...
public class ChessboardFinalisedModel implements IPerspectiveAwareModel 
{
	...
	/**
	 * We return a list of quads here which is used to draw the chessboard. We do this by getting <br>
	 * the list of quads for the base model (the chessboard itself), then adding an extra quad for every <br>
	 * piece on the chessboard. The number of pieces was provided to the constructor of the finalized model.
	 *
	 * @param side  which side: north, east, south, west, up, down, or null.
	 * @return the list of quads to be rendered.
	 * @see for more information about <code>side</code>: <a href="http://minecraft.gamepedia.com/Block_models#Item_models">Block models</a>
	 */
	...
	public Pair<? extends IBakedModel, Matrix4f> handlePerspective(ItemCameraTransforms.TransformType cameraTransformType) 
	{
		...
		{
			// If the parent model isn't an IPerspectiveAware, we'll need to generate the 
	...
	private List<BakedQuad> getChessPiecesQuads(int numberOfPieces)
	{
		 ...
		 * (i.e. north<-->south thickness), are centered around the z=0.5 plane.
		 */
		...
		// See more information here - http://greyminecraftcoder.blogspot.co.at/2014/12/blocks-18.html
		...
		for (int i = 0; i < numberOfPieces; ++i) 
		{
			 ...
			 *  which leads to "z-fighting" where the two quads fight each other to be on top. Looks awful.
			 */
	...
	/**
     *  Creates a baked quad for the given face. <p>
     * 
     *  When you are directly looking at the face, the quad is centred at [centreLR, centreUD]. <br>
     *  The left<->right "width" of the face is width, the bottom<-->top "height" is height. <br>
     *  The amount that the quad is displaced towards the viewer i.e. perpendicular to <br>
     *  the flat face you can see is forwardDisplacement. <p>
     * 
     *  For example, for an EAST face, a value of 0.00 lies directly on the EAST face of the cube. <br>
     *  a value of 0.01 lies slightly to the east of the EAST face (at x=1.01) <br>
     *  and a value of -0.01 lies slightly to the west of the EAST face (at x=0.99). <p>
     *  
     *  The orientation of the faces is as per the diagram on this <a href="http://greyminecraftcoder.blogspot.com.au/2014/12/block-models-texturing-quads-faces.html">page</a>.
     *  <br> Read this <a href="http://greyminecraftcoder.blogspot.co.at/2014/12/the-tessellator-and-worldrenderer-18.html">page</a> to learn more about how to draw a textured quad
     *  
	...
	private int[] vertexToInts(float x, float y, float z, int color, TextureAtlasSprite texture, float u, float v)
	{
		return new int[] 
		{
				...
				Float.floatToRawIntBits(z), color,
